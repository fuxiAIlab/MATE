...
# from Periodic_Self_Attention import PeriodicSelfAttention
from mate import PeriodicSelfAttention
from Baseline.LSTM import LSTM
from Baseline.markov_chain import MarkovChain
...
class ModelTrainer:
    def __init__(self, log_dir=hp.log_file, model_file=hp.model_file, output_file=None):
        ...
        self.model = LSTM()  # PeriodicSelfAttention()  #
        self.mc = MarkovChain(order=1, class_num=self.model.class_num, lastlen=hp.lastlen)
            ...
            with tf.Session(graph=self.model.graph) as session:
                ...
                for batch_i, (x_batch, y_batch, time_batch, time_label_batch, portrait_batch, role_id_batch,
                    ...
                    with tf.name_scope('loss'):
                        # Training
                        self.mc.update_table(x_batch, y_batch)
                        logits_, loss_ = self.mc.get_loss(x_batch, y_batch)
